...
STORY STATE (dialogue, emails, events to wire in):
${JSON.stringify(storyState, null, 2)}
...
YOUR TASK — ASSEMBLY SEQUENCE:
a. Load current game.html as base
b. Apply Engine changes first (structural — from recent engine round logs)
c. Integrate Story content (dialogue data, email data, NPC definitions from story-state.json)
d. Wire Art assets (update ASSET_MANIFEST, add sprite references)
e. Connect systems (new systems call into existing ones)
f. Verify all protected system functions are still present
...
- After </html>, include build report between markers:
  ...
  if (validationErrors.length > 0) {
    broadcast({ type: 'validation_failed', round, errors: validationErrors });
    broadcast({ type: 'developer_retry', round });
    const retryPrompt = atlasIntPrompt + `\n\nVALIDATION FAILED:\n${validationErrors.join('\n')}\n\nFix and output corrected game.html.`;
    try {
      atlasOutput = await runAgent('atlas-novak', atlas.model, retryPrompt, 600000);
      const retry = extractHtmlAndReport(atlasOutput);
      if (!retry.gameHtml) {
...
Then provide your detailed findings.`;
  ...
  if (verdict === 'ship') {
    ...
    broadcast({ type: 'developer_retry', round });
    const retryPrompt = atlasIntPrompt + `\n\nQA REJECTED THE BUILD:\n${beaOutput}\n\nFix the issues Bea identified. Output corrected game.html.`;
...
Include build report after </html> between <!-- BUILD_REPORT_START --> and <!-- BUILD_REPORT_END --> markers.`;
  ...
  if (!gameHtml) {
    broadcast({ type: 'agent_error', round, agentId: 'atlas-novak', error: 'Hotfix output is not valid HTML' });
