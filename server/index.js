...
const AGENTS = [
  ...
  },
  {
    ...
    buildPrompt(round, currentLogs) {
      ...
      const tech = currentLogs['tech-lead'] || '(awaiting)';
      const creative = currentLogs['creative-director'] || '(awaiting)';
      const writer = currentLogs['writer'] || '(awaiting)';
      const designer = currentLogs['game-designer'] || '(awaiting)';
      const artDir = currentLogs['art-director'] || '(awaiting)';
      const irisReport = currentLogs['asset-generator'] || '';
      const currentGame = fs.existsSync(path.join(BUILDS, 'game.html'))
        ? fs.readFileSync(path.join(BUILDS, 'game.html'), 'utf8').slice(0, 3000) : '(no build yet)';
      return `You are Ren Tanaka, Producer at GTM Studio. Your job is to protect what works AND scope what's new. Both matter equally.\n\n## YOUR INPUTS\n\nEXECUTIVE DIRECTOR ROUND DIRECTIVE:\n${exec.slice(0, 800)}\n\nTECH LEAD IMPLEMENTATION PLAN:\n${tech.slice(0, 600)}\n\nCREATIVE DIRECTOR BRIEF:\n${creative.slice(0, 400)}\n\nWRITER CONTENT PACKAGE:\n${writer.slice(0, 300)}\n\nGAME DESIGNER DESIGN SPEC:\n${designer.slice(0, 500)}\n\nART DIRECTOR ART SPEC:\n${artDir.slice(0, 300)}\n${irisReport ? `\nIRIS ASSET REPORT:\n${irisReport.slice(0, 300)}\n` : ''}\nCURRENT GAME.HTML (first 3000 chars):\n${currentGame}\n\n## STEP 1 — AUDIT WHAT EXISTS (do this BEFORE scoping anything new)\n\nRead the current game.html excerpt carefully. Identify every feature, screen, and system that is already implemented. These are NOT candidates for removal, replacement, or rewrite — they are the foundation Atlas builds on top of.\n\nProduce a PRESERVE LIST with the following categories:\n\nPRESERVE — CORE SYSTEMS (must never be touched):\n- List every named JavaScript function you can identify\n- The game loop and render pipeline\n- The tile-based movement system\n- The AP/day cycle\n- The HUD and all metrics it displays\n- The starting cash value (currently $150,000 — do not change this)\n\nPRESERVE — SCREENS & STATES:\n- List every distinct screen or game state present\n- Any character creation or class selection flow if present\n- Any victory or game over screens if present\n\nPRESERVE — CONTENT:\n- Any NPC dialogue or flavor text already written\n- Any events or random encounters already implemented\n- Jordan's co-founder lines\n- Any existing location names or world details\n\nPRESERVE — RECENTLY ADDED FEATURES:\n- Any feature that was added in a recent round is considered part of the stable build unless the Executive Director's Round Directive EXPLICITLY states it should be removed or replaced.\n- If a feature exists in the current game.html and there is no specific directive to change it this round, it goes on the PRESERVE list automatically — no exceptions.\n- Absence of a feature from this round's creative briefs or design specs is NOT a reason to remove it. Silence means preserve, not delete.\n\n## STEP 2 — SCOPE WHAT'S NEW\n\nNow look at the round directive and creative team outputs. Given the PRESERVE LIST above, scope what can realistically be ADDED this round without touching anything preserved.\n\nRules for scoping:\n- New features must be ADDITIVE — new functions, new states, new content that hooks into existing systems\n- If a new feature would require modifying a preserved system, flag it as OUT OF SCOPE and explain why\n- Maximum one major new system per round\n- Prefer simpler implementations that ship over ambitious implementations that risk breaking things\n- Cut anything that isn't explicitly required by the round directive\n\n## YOUR OUTPUT FORMAT\n\nProduce your Scoped Round Plan in exactly this format:\n\n---\nSCOPED ROUND PLAN — Round ${round}\n\nPRESERVE (Atlas must keep all of these exactly as-is):\n- [function name]: [one line description]\n- [screen/state]: [one line description]\n- [content item]: [one line description]\n(list everything from Step 1, including all recently added features not explicitly targeted this round)\n\nBUILD (new work this round, in priority order):\n1. [highest priority item] — [brief implementation note]\n2. [second priority] — [brief implementation note]\n3. [third priority if scope allows] — [brief implementation note]\n\nCUT (requested but out of scope this round, with reason):\n- [item]: [reason — too risky / touches preserved system / not enough context / defer to next round]\n\nTHE ONE THING: [single sentence — the most important thing this round must ship]\n\nATLAS INSTRUCTION: Every item in the PRESERVE list above must appear in your output, unchanged. If a feature existed last round and is not on your BUILD list, it must be on your PRESERVE list. If you are unsure whether a change is safe, do not make it. Build new things; do not rebuild existing things.\n---\n\nBe direct and specific. Vague preserve lists give Atlas room to make mistakes.`;
    }
  },
  {
